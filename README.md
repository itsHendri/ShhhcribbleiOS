...
1. Clone and open in Xcode:
   ...
   ```
2. In Xcode, select your iPhone as destination, then **Cmd-R**. **Replace my signing identifiers in `project.yml` with your own** before building (see "Adapting to your Apple ID" below).
...
## Adapting to your Apple ID

`project.yml` is regenerated by `xcodegen` on every change, which **blanks out the development-team selection** in the pbxproj. To stop the manual "set team in Xcode every time" loop, my signing identity is pinned directly in `project.yml`. If you cloned (or merged) this repo, swap these for your own values **before** running `xcodegen generate`:

| Setting | What it is | Replace with |
|---|---|---|
| `options.bundleIdPrefix` | Bundle-ID prefix (line 3) | Your reverse-DNS prefix, e.g. `com.yourname` |
| `PRODUCT_BUNDLE_IDENTIFIER` (×3 targets) | Per-target bundle IDs | `com.yourname.shhhcribble`, `…shhhcribble.shared`, `…shhhcribble.widget` |
| `DEVELOPMENT_TEAM` (×3 targets) | Apple Developer Team ID | Yours (10-char ID, found in Xcode → Settings → Accounts, or the Apple Developer portal) |
| `CFBundleURLName` (line 66) | Custom URL scheme owner | `com.yourname.shhhcribble` |
...
`L9T3PX7HVH` (my Personal Team) and `com.hendrivanniekerk.*` are the values currently checked in. They will fail to sign on any other Apple ID.
